<template>
    <div style="padding-right: 7%; width: 80%;" id="customer-registrations-main-body">
        ...
        <div class="content">
            ...
            <button class="update-btn" @click="modifySchedule()" v-bind:disabled="isUpdateButtonDisabled"> Update
                Schedule </button>
        </div>
    </div>
</template>
...
export default {
    ...
    // beforeMount(){
    //     try{
    //         const response = client.get('/schedule');
    //         this.curr_open = response.openingHour;
    ...
    async created() {
        try {
            ...
            this.curr_open = response.data.openingHours;
            this.curr_close = response.data.closingHours;
        } catch (e) {
            this.curr_open = "N/A";
            this.curr_close = "N/A";
        }
